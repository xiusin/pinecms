...
func (c *MemberController) List() {
	...
	table := helper.Datagrid("member_list_datagrid", "/b/user/list", helper.EasyuiOptions{
		...
		"操作": {"field": "id", "width": "25", "index": "6", "formatter": "optFormatter"},
	})
...
func (c *MemberController) Add() {
	var d tables.Member
	if err := c.Ctx().BindForm(&d); err != nil {
		helper.Ajax("参数错误:" + err.Error(), 1, c.Ctx())
		return
	}
	if d.Password == "" || d.Account == "" {
		helper.Ajax("密码或账户名不能为空" , 1, c.Ctx())
		return
	}
	m := models.NewMemberModel()
	if m.Exist(d.Account) {
		helper.Ajax("已经存在同名账户, 请换一个再试" , 1, c.Ctx())
		return
	}
	d.CreatedAt = time.Now().In(helper.GetLocation())
	if m.Add(&d) > 0 {
		helper.Ajax("添加成功", 0, c.Ctx())
	} else {
...
func (c *MemberController) Edit() {
	...
	if models.NewMemberModel().Edit(d.Id, &d) {
