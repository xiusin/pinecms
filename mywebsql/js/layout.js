...
var layoutState = {
	...
	options: {
		layoutName:	'mywebsql' // default name (optional)
	...
	*/
	...
	,	domain:		''
	,	path:		''
	,	expires:	'60'	// 'days' to keep cookie - leave blank for 'session cookie'
	,	secure:		false
	}
...
,	clear: function (layoutName) {
		this.save( layoutName, 'layout', { expires: -1 });
	}
...
,	save: function (layoutName, keys, opts) {
		...
		if (clear) {
			...
			$.cookies.set((layoutName || o.layoutName), params, {path: EXTERNAL_PATH, hoursToLive: COOKIE_LIFETIME});
		}
		else {
			this.data = readState( layout, keys ); // read current panes-state
			$.cookies.set((layoutName || o.layoutName), encodeURIComponent(JSON.stringify(this.data)) + params, {path: EXTERNAL_PATH, hoursToLive: COOKIE_LIFETIME});
		}
		return this.data;
...
,	load: function (layoutName) {
		...
		var data = {};
		ck = $.cookies.get(layoutName);
		if (ck)
			data = JSON.parse(decodeURIComponent(ck));
