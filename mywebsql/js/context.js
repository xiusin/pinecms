	...
	$.contextMenu = {
		...
		create: function(menu,opts) {
			...
			if (typeof menu=="string") {
				cmenu.menu = $(menu);
				//cmenu.menu.find("li:has(ul)").find("a:first").append("&nbsp;&nbsp;&nbsp;"); // causes problem with objectlist refresh
			} 
		...
		show: function(t,e) {
			...
			if (cmenu.beforeShow()!==false) {
				...
				if (cmenu.menuFunction) {
					...
					cmenu.menu = cmenu.menuFunction(cmenu,t,e);
					if (! cmenu.menu)
						return false;	// ok we don't need a menu for this target
					cmenu.menu.css({display:'none'});
					$(cmenu.appendTo).append(cmenu.menu);
				}
				var $c = cmenu.menu;
				/* custom changes to make it work for div based menus */
