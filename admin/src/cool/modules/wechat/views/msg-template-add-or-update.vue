	...
	<cl-dialog title="模板配置" :close-on-click-modal="false" v-model="visible">
		<el-form :model="dataForm" :rules="dataRule" ref="dataForm" label-width="100px" size="mini">
			<el-form-item label="标题" prop="title" size="mini">
				<el-input v-model="dataForm.title" placeholder="标题" size="mini" />
			</el-form-item>
			<el-form-item label="链接" prop="url" size="mini">
				<el-input v-model="dataForm.url" placeholder="跳转链接" size="mini" />
			</el-form-item>
			<div>
				<el-form-item label="小程序appid" prop="miniprogram.appid" size="mini">
					<el-input
						v-model="dataForm.miniprogram.appid"
				...
				<el-form-item label="小程序路径" prop="miniprogram.pagePath">
					<el-input
						v-model="dataForm.miniprogram.pagePath"
						placeholder="小程序pagePath"
						size="mini"
					/>
				</el-form-item>
			</div>
			<el-row>
				<el-col :span="16">
					<el-form-item label="模版名称" prop="name">
						<el-input v-model="dataForm.name" placeholder="模版名称" size="mini" />
					</el-form-item>
				</el-col>
				<el-col :span="8">
			...
			<div class="form-group-area">
				<el-card class="box-card" :closable="false" shadow="hover">
					<div slot="header" class="clearfix">
						<el-alert show-icon type="warning" :closable="false" effect="dark"
							>消息填充数据，请对照模板内容填写</el-alert
						>
					</div>
					<div style="padding: 5px; font-size: 12px; background-color: #dfe1e5">
						<code>
							<pre>{{ dataForm.content }}</pre>
						</code>
					</div>
				</el-card>
				...
				<el-row v-for="(item, index) in dataForm.data" :key="item.name">
					<el-col :span="16">
						<el-form-item
							...
							:rules="[
								{ required: true, message: '填充内容不得为空', trigger: 'blur' }
							]"
						>
							<el-input
								type="textarea"
	...
	data() {
		return {
			...
			dataRule: {
				...
				data: [{ required: true, message: "内容不能为空", trigger: "blur" }],
