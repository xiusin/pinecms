	...
	<div class="mod-config">
		<el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
			...
			<el-form-item>
				...
				<el-button
					v-if="isAuth('wx:wxuser:save')"
					type="primary"
					@click="$refs.wxUserTagging.init('tagging')"
				...
				<el-button
					v-if="isAuth('wx:wxuser:save')"
					type="primary"
					@click="$refs.wxUserTagging.init('untagging')"
					:disabled="dataListSelections.length <= 0"
					>解绑标签</el-button
				>
				<el-button
					v-if="isAuth('wx:wxuser:delete')"
					type="danger"
					@click="deleteHandle()"
					:disabled="dataListSelections.length <= 0"
					>批量删除</el-button
			...
			<el-form-item class="fr">
				<el-button
					...
					type="success"
					@click="$refs.wxUserTagsEditor.show()"
					>标签管理</el-button
				>
				<el-button icon="el-icon-sort" type="success" @click="syncWxUsers()"
					>同步粉丝</el-button
				>
			</el-form-item>
		</el-form>
		...
		>
			...
			>
				<template #default="scope">
					<span v-for="tagid in scope.row.tagidList" :key="tagid"
						>{{ getTagName(tagid) }}
					</span>
				</template>
			</el-table-column>
			...
			>
				<template #default="scope">{{
					$moment(scope.row.subscribeTime).calendar()
				}}</template>
			</el-table-column>
			<el-table-column
				prop="qrSceneStr"
			...
			>
				<template #default="scope">
					<el-button type="text" size="small" @click="deleteHandle(scope.row.openid)"
						>删除</el-button
					>
				</template>
			</el-table-column>
		...
		<wx-user-tags-manager
			ref="wxUserTagsEditor"
			:visible="showWxUserTagsEditor"
			@close="showWxUserTagsEditor = false"
		/>
		<wx-user-tagging ref="wxUserTagging" :wxUsers="dataListSelections" />
	</div>
</template>
