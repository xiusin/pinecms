	...
	<div>
		<cl-crud :ref="setRefs('crud')" @load="onLoad">
			<el-row type="flex">
				...
				<cl-refresh-btn />
				<el-button size="mini">
					<el-link type="primary" icon="el-icon-link" size="mini" style="font-size: 12px" target="_blank" href="https://kf.qq.com/faq/170209E3InyI170209nIF7RJ.html">模板管理指引</el-link>
				</el-button>
				<cl-flex1 />
				<cl-search-key />
			</el-row>
			...
			<el-row>
				<cl-table v-bind="table">
					...
					<template #slot-copy="{ scope }">
						<el-button size="mini" @click="copyHandle(scope.row)" type="text">复制</el-button>
					</template>
				</cl-table>
			</el-row>
...
export default defineComponent({
	...
	setup() {
		...
		function sync() {
			ElMessageBox.confirm("是否要同步微信模板, 此操作为增量更新？", "提示", { type: "warning" }).then(() => {
				service.wechat.template.sync().then(() => {
		...
		function send() {
			service.wechat.template.send()
		}

		function copyHandle(row: any){
			service.wechat.template.info({"id": row.id}).then((data: any) => {
				data.name+='_COPY'
				data.id = 0;
