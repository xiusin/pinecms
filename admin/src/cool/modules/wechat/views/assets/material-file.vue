  ...
  <div class="mod-menu">
    ...
    <div v-loading="dataListLoading">
      <div class="card" v-for="item in dataList" :key="item.media_id" @click="onSelect(item)">
        <el-image v-if="fileType=='image'" class="card-image" :src="'http://read.html5.qq.com/image?src=forum&q=5&r=1&imgflag=8&imageUrl=' + item.url" fit="contain"></el-image>
        ...
        <div class="card-footer">
          ...
          <div class="flex justify-between align-center" v-show="!selectMode">
            <el-button size="mini" type="text" icon="el-icon-copy-document">复制media_id</el-button>
            <el-button size="mini" type="text" icon="el-icon-delete" @click="deleteHandle(item.media_id)">删除</el-button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.service.wechat.material.page({
        page: this.pageIndex,
        type: this.fileType,
        size: this.pageSize,
