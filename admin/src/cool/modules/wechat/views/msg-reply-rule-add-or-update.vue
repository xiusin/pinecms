	...
	>
		<el-form :model="dataForm" :rules="dataRule" ref="dataForm" label-width="80px">
			...
			<el-form-item label="匹配词" prop="matchValue">
				<tags-editor v-model="dataForm.matchValue" />
			</el-form-item>
			<el-row>
				<el-col :span="12">
					<el-form-item label="作用范围" prop="appid">
						<el-select v-model="dataForm.appid" placeholder="作用范围">
							...
							<el-option label="当前公众号" :value="selectedAppid" />
						</el-select>
					</el-form-item>
				</el-col>
				<el-col :span="12">
			...
			<el-row>
				<el-col :span="12">
					<el-form-item label="生效时间" prop="effectTimeStart">
						<el-time-picker
							v-model="dataForm.effectTimeStart"
				...
				<el-col :span="12">
					<el-form-item label="失效时间" prop="effectTimeEnd">
						<el-time-picker v-model="dataForm.effectTimeEnd" value-format="HH:mm:ss" />
					</el-form-item>
				</el-col>
			</el-row>
			<el-form-item label="回复内容" prop="replyContent">
				...
				<el-button type="text" v-show="assetsType" @click="assetsSelectorVisible = true">
					从素材库中选择<span
						v-if="
							'miniprogrampage' == dataForm.replyType || 'music' == dataForm.replyType
						"
						>缩略图</span
					>
				</el-button>
			...
			<el-form-item label="备注说明" prop="desc">
				<el-input v-model="dataForm.desc" placeholder="备注说明" />
			</el-form-item>
		</el-form>
...
export default {
	components: {
		...
		AssetsSelector: () => import("./assets/assets-selector.vue")
	},
	data() {
		return {
			visible: false,
			...
			dataRule: {
				...
				status: [{ required: true, message: "是否有效不能为空", trigger: "blur" }],
				effectTimeStart: [
	...
	methods: {
		...
		dataFormSubmit() {
			this.$refs["dataForm"].validate((valid) => {
				if (valid) {
					this.$http({
						url: this.$http.adornUrl(
							`/manage/msgReplyRule/${!this.dataForm.ruleId ? "save" : "update"}`
						),
						method: "post",
		...
		onReplyTypeChange(value) {
			if ("miniprogrampage" == value) {
				...
				let demo = {
					head_content: "开头文字",
					list: [
						{ id: "菜单1ID", content: "菜单2内容" },
						{ id: "菜单2ID", content: "菜单2内容" },
						{ id: "菜单nID", content: "菜单n内容" }
					],
					tail_content: "结尾文字"
				};
				this.dataForm.replyContent = JSON.stringify(demo, null, 4);
		...
		onAssetsSelect(assetsInfo) {
			if (
				this.dataForm.replyType == "miniprogrampage" ||
				this.dataForm.replyType == "music"
			) {
